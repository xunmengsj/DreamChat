angular.module('dreamChatApp').factory('server', ['$cacheFactory', '$q', '$http', 'socket', function($cacheFactory, $q, $http, socket) {
    var cache = window.cache = $cacheFactory('dreamChat')
    socket.on('dreamChat', function(data) {
        switch (data.action) {
            case 'getRoom':
                if (data._roomId) {
